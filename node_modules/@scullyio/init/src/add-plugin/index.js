...
const addPlugin = (options) => (tree, context) => {
    ...
    const pathName = core_1.strings.dasherize(`${sourceRoot}/scully-plugins/`);
    return utils_1.applyWithOverwrite(schematics_1.url(`../files/plugin/${options.pluginType}`), [
...
const registerPlugin = (options) => (tree, context) => {
    ...
    scullyConfig = `require('./scully-plugins/${core_1.strings.dasherize(options.name)}.plugin.js');\n${scullyConfig}`;
